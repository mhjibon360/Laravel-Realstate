    ...
    <div class="row profile-body">
        ...
        <div class="col-md-12 col-xl-12 middle-wrapper">
            <div class="row">
                <div class="card mt-4">
                    <div class="card-body">
                        <h5 class="card-title text-capitalize ">List of property type</h5>
                        <hr>
                        <div class="table-responsive">
                            <table id="dataTableExample" class="table">
                                ...
                                <tbody>
                                    @foreach ($allptype as $item)
                                        <tr>
                                            <td>{{ $loop->index + 1 }}</td>
                                            <td>{{ $item->property_typename }}</td>
                                            <td>{{ $item->created_at->format('Y/M/d') }}</td>
                                            <td>
                                                <a href="{{ route('admin.property-type.edit', $item->id) }}"
                                                    class="btn btn-inverse-warning mb-1 mb-md-0">Edit</a>
                                                <form action="{{ route('admin.property-type.destroy', $item->id) }}"
