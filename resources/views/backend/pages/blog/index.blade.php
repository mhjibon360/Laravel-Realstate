    ...
    <div class="row profile-body">
        ...
        <div class="col-md-12 col-xl-12 middle-wrapper">
            <div class="row">
                <div class="card mt-4">
                    <div class="card-body">
                        <h5 class="card-title text-capitalize ">List of Blog post </h5>
                        <hr>
                                ...
                                <tbody>
                                    @foreach ($allblog as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->title }}</td>
                                            <td>
                                                <img src="{{ asset($item->thumbnail) }}" class=" img-fluid img-thumbnail"
                                                    style="height: 60px;width:60px;" alt="">
                                            </td>
                                            <td><span>{{ $item->blogcategory->category_name }}</span> </td>
                                            <td>
                                            ...
                                            <td>
                                                <input data-id="{{ $item->id }}" class="status_blog post_toggle_class"
                                            ...
                                            <td>
                                                <a href="{{ route('admin.blog-post.edit', $item->id) }}"
                                                ...
                                                <a href="" class="btn btn-inverse-info mb-1 mb-md-0"><i
...
@push('admin_script')
    ...
    <script>
        $(document).ready(function() {
            ...
            // featured blog post
            $('.status_blog').change(function() {
                var status = $(this).prop('checked') == true ? 1 : 0;
                var id = $(this).data('id');

                $.ajax({
                    type: "GET",
                    url: route('admin.blogpost.status'),
                    data: {
                        status: status,
                        id: id,
                    },
