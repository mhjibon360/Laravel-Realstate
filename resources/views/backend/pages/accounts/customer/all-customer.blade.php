...
@section('title', 'all customer list')
@section('content')
        ...
        <div class="col-md-12 col-xl-12 middle-wrapper">
            <div class="row">
                <div class="card mt-4">
                    ...
                    <div class="card-body">
                        <h5 class="card-title text-capitalize ">List of customer </h5>
                        <hr>
                                ...
                                <tbody>
                                    @foreach ($allcustomer as $user)
                                        <tr>
                                            ...
                                            <td>
                                                <input data-id="{{ $user->id }}" class="status_customer_toggle_class"
                                            ...
                                            <td>
                                                <a href="{{ route('admin.edit.customer.account', $user->id) }}"
                                                    class="btn btn-inverse-warning mb-1 mb-md-0"><i
                                                        class="fa-solid fa-pen-to-square"></i></a>
                                                <form action="{{ route('admin.delete.customer.account', $user->id) }}"
...
@push('admin_script')
    ...
    <script>
        $(document).ready(function() {
            // featured customer
            $('.status_customer_toggle_class').change(function() {
                var status = $(this).prop('checked') ? 'active' : 'deactive';
                var id = $(this).data('id');

                $.ajax({
                    type: "POST",
                    url: route('admin.status.customer.account'),
                    data: {
                        _token: "{{ csrf_token() }}",
