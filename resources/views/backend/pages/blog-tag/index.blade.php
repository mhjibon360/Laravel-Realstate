    ...
    <div class="row profile-body">
        ...
        <div class="col-md-12 col-xl-12 middle-wrapper">
            <div class="row">
                <div class="card mt-4">
                    <div class="card-body">
                        <h5 class="card-title text-capitalize ">List of Blog Tag</h5>
                        <hr>
                        <div class="table-responsive">
                            <table id="dataTableExample" class="table">
                                ...
                                <tbody>
                                    @foreach ($allblogtag as $item)
                                        <tr>
                                            <td>{{ $loop->index + 1 }}</td>
                                            <td>{{ $item->tag_name }}</td>
                                            <td>{{ $item->created_at->format('d-M-Y') }}</td>
                                            <td>
                                                <a href="{{ route('admin.blog-tag.edit', $item->id) }}"
                                                    class="btn btn-inverse-warning mb-1 mb-md-0">Edit</a>
                                                <form action="{{ route('admin.blog-tag.destroy', $item->id) }}"
